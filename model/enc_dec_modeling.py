...
class EncDecModel(nn.Module):
    ...
    def print_prompt_embeds(self):
        print(self.encoder.prompt_embeds.weight[0, :])
        # print(self.decoder.prompt_embeds.weight[0, :])
        # exit()
...
def enc_dec_get_params_for_prompt_optimization(args, module: nn.Module, wd=0.0, do_distill=False):
    # return params
    params = []
    if args.tuning_type == 'bias':
        for module_ in module.modules():
            param = [p for n, p in list(module_._parameters.items())
                    if p is not None and n == 'bias']
            if param: params.append({'params': param, 'weight_decay': wd})

    for t in module.named_modules():
        if ('up_p' in t[0] or 'down_p' in t[0]) and args.tuning_type=='adapter':
            params.append({'params': [p for p in list(t[1]._parameters.values()) if p is not None], 'weight_decay': wd})

        if 'prompt' in t[0] and args.tuning_type=='prompt':
            params.append({'params': [p for p in list(t[1]._parameters.values()) if p is not None], 'weight_decay': wd})
