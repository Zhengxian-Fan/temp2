...
from T5Dataset import XNLIDataset, T5Dataset
...
def evaluate(args, tokenizer: EncDecTokenizer, data_config, eval_dataset, eval_data_loader, device, mode='dev'):
    """Evaluation."""
    ...
    for model_batch, no_model_batch in eval_data_loader:
        gathered_idx = [torch.zeros_like(no_model_batch["lang"]) for _ in range(mpu.get_data_parallel_world_size())]
        torch.distributed.all_gather(gathered_idx, no_model_batch["lang"].contiguous(), mpu.get_data_parallel_group())
        all_langs.extend(gathered_idx)
...
def main():
    ...
    if args.do_eval:
        ...
        loss, acc = eval_func(args, tokenizer, data_config, eval_dataset, eval_dataloader, device, mode="test")
