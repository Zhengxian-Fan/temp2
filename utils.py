...
def similarity_search(x, y, dim):
  num = x.shape[0]
  idx = faiss.IndexFlatL2(dim)
  faiss.normalize_L2(x)
  faiss.normalize_L2(y)
  idx.add(x)
  dists, ids = idx.search(y, 1)
  return ids
...
def get_prompt_name(checkpoints_path, iteration, release=False, zero=False):
    if release:
        d = 'release'
    else:
        d = 'iter_{:07d}'.format(iteration)
    if zero:
        dp_rank = mpu.get_data_parallel_rank()
        d += '_zero_dp_rank_{}'.format(dp_rank)
    return os.path.join(checkpoints_path, d, 'prompt.pt')
...
def save_prompt(iteration, model, optimizer,
                    lr_scheduler, args, save_dir=None):
    """Save a model checkpoint."""
    # Only rank zer0 of the data parallel writes to the disk.
    if isinstance(model, torchDDP):
    ...
    if mpu.get_data_parallel_rank() == 0 and torch.distributed.get_rank() == 0:
        checkpoint_name = os.path.join(save_dir, 'prompt.pt')
        print('global rank {} is saving checkpoint at iteration {:7d} to {}'.
            format(torch.distributed.get_rank(), iteration, checkpoint_name))
        ...
        sd['model'] = model.state_dict()
        prompt = {}
        if 'module.encoder.prompt_embeds.weight' in sd['model']:
            prompt['encoder.prompt_embeds.weight'] = sd['model']['module.encoder.prompt_embeds.weight']
        if 'module.encoder.align_prompt_embeds.weight' in sd['model']:
            prompt['encoder.align_prompt_embeds.weight'] = sd['model']['module.encoder.align_prompt_embeds.weight']
        # prompt['decoder.prompt_embeds.weight'] = sd['model']['module.decoder.prompt_embeds.weight']
        ensure_directory_exists(checkpoint_name)
        torch.save(prompt, checkpoint_name)
        print('  successfully saved {}'.format(checkpoint_name))
