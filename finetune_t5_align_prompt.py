...
from T5ParallelDataset import DictDataset, T5Dataset, AllNLIDataset, TatoebaDataset
...
def get_optimizer(model, args, prompt_config=None):
    ...
    while isinstance(model, (DDP, FP16_Module)):
        model = model.module
    if args.prompt_tune:
        param_groups = enc_dec_get_params_for_prompt_optimization(model, args.wd)
    else:
...
# import transformers
# model2 = transformers.MT5EncoderModel.from_pretrained('google/mt5-large').to('cuda')
# model2.eval()
# model2.half()
...
def forward_step(args, model_batch, no_model_batch, model, device, keep_enc_hidden=False, do_infer=False):
    ...
    hidden_state = enc_outputs["encoder_last_hidden_state"]
    attention_mask = model_batch["enc_attention_mask"][:,0,0,:]
    pooled_output = (hidden_state * attention_mask.unsqueeze(2).float()).sum(dim=1) / attention_mask.sum(dim=1).view(-1, 1).float()
    # print("ps",pooled_output.size())
    # print(attention_mask.size())
    #attention_mask = attention_mask.sum(dim=1).long() -1
    #pooled_output = hidden_state[torch.arange(hidden_state.size(0)),attention_mask]
    # print(pooled_output.size())
    #hidden_state = model2(input_ids=model_batch["enc_input_ids"], attention_mask=model_batch["enc_attention_mask"][:,0,0,:])['last_hidden_state']
    # print(hidden_state1-hidden_state)
...
def train(args, data_config, tokenizer, model, optimizer, lr_scheduler,
    ...
    for e in range(args.epochs):
        ...
        for model_batch, no_model_batch in train_dataloader:
            ...
            pooled_output = forward_step(args, model_batch, no_model_batch, model, device)

            labels = no_model_batch["labels"]
            all_labels = [torch.zeros_like(labels) for _ in range(mpu.get_data_parallel_world_size())]
            torch.distributed.all_gather(all_labels, labels.contiguous(), mpu.get_data_parallel_group())
            all_labels = torch.cat(all_labels, dim=0).squeeze(1)

            all_pooled_output = [torch.zeros_like(pooled_output) for _ in range(mpu.get_data_parallel_world_size())]
            torch.distributed.all_gather(all_pooled_output, pooled_output.contiguous(), mpu.get_data_parallel_group())
            all_pooled_output[mpu.get_data_parallel_rank()] = pooled_output
            all_pooled_output = torch.cat(all_pooled_output, dim=0).squeeze(1)
            loss = batch_all_triplet_loss(all_labels, all_pooled_output, 0.3)[0]
            ...
            if args.save and args.save_interval and global_step % args.save_interval == 0 and step % args.gradient_accumulation_steps == 0:
                save_prompt(global_step, model, optimizer, lr_scheduler, args, save_dir=os.path.join(args.save, "step_{}".format(global_step)))

            # Evaluation
            if global_step > 2000 and args.eval_interval and global_step % args.eval_interval == 0 and step % args.gradient_accumulation_steps == 0 and args.do_valid:
...
def evaluate(args, tokenizer: EncDecTokenizer, data_config, eval_dataset, eval_data_loader, model, device, mode='dev'):
    ...
    with torch.no_grad():
        for model_batch, no_model_batch in eval_data_loader:
            pooled_output = forward_step(args, model_batch, no_model_batch, model, device, do_infer=(mode=="infer"))
            all_pooled_output = [torch.zeros_like(pooled_output) for _ in range(mpu.get_data_parallel_world_size())]
            torch.distributed.all_gather(all_pooled_output, pooled_output.contiguous(), mpu.get_data_parallel_group())
            all_pooled_output[mpu.get_data_parallel_rank()] = pooled_output
...
def encode(args, eval_data_loader, model, device, mode='dev'):
    """Evaluation."""
    ...
    with torch.no_grad():
        for model_batch, no_model_batch in eval_data_loader:
            pooled_output = forward_step(args, model_batch, no_model_batch, model, device, do_infer=(mode=="infer"))
            #batch_pooled_output = [torch.zeros_like(pooled_output) for _ in range(mpu.get_data_parallel_world_size())]
            #torch.distributed.all_gather(batch_pooled_output, pooled_output.contiguous(), mpu.get_data_parallel_group())
            #batch_pooled_output = torch.cat(batch_pooled_output, dim=0).cpu().numpy().astype(np.float32)
            #all_pooled_output.append(batch_pooled_output)
            all_pooled_output.append(pooled_output.cpu().numpy().astype(np.float32))

    all_pooled_output = np.concatenate(all_pooled_output, axis=0)
    # print(all_pooled_output.shape)
...
def load_data(args, data_config, data_type, tokenizer, prompt_config=None, ratio=1, drop_last=True, do_infer=False, filename=None):

    if filename:
        path = os.path.join(data_config[args.data_name]['path'], filename)
    else: 
        path = data_config[args.data_name]['path']
    ...
    if data_type == 'train':
        ...
        sampler = torch.utils.data.SequentialSampler(dataset)
        global_batch_size = len(dataset) // world_size * world_size
...
def main():
    ...
    data_config = {
        "dict": {
            ...
            "path": os.path.join(args.data_path, "dict/all.txt"),
        },
        "allnli": {
            "dataset": AllNLIDataset,
            "eval_func": evaluate,
            "cache_path": None,
            "infer_func": infer_xnli,
            "path": os.path.join(args.data_path, "AllNLI.tsv.gz"),
        },
        "tatoeba": {
            "dataset": TatoebaDataset,
            "eval_func": evaluate,
            "cache_path": None,
            "path": os.path.join(args.data_path, "tatoeba"),
        },
    }
    ...
    if args.do_eval:
        if args.data_name == 'tatoeba':
            langs = args.eval_lang.split(',')
            accs = []
            for lang in langs:
                l1_filename = '{}-en.{}'.format(lang, lang)
                l2_filename = '{}-en.en'.format(lang)
                # l1_filename = 'zh-en.zh'
                # l2_filename = 'zh-en.en'
                eval_dataloader1, _ = load_data(args, data_config, 'dev', tokenizer, prompt_config, ratio=1.0, filename=l1_filename)
                eval_dataloader2, _ = load_data(args, data_config, 'dev', tokenizer, prompt_config, ratio=1.0, filename=l2_filename)
                with torch.no_grad():
                    l1_embeds = encode(args, eval_dataloader1, model, device)
                    l2_embeds = encode(args, eval_dataloader2, model, device)
                ids = similarity_search(l1_embeds, l2_embeds, l1_embeds.shape[1])
                acc = accuracy_score(np.squeeze(ids), np.arange(0, l1_embeds.shape[0]))
                accs.append(acc)
                log_string = "Tatoeba result: lang: {} | acc: {:.4}".format(lang, acc)
                print_rank_0(log_string)
                save_rank_0(args, log_string)
            log_string = "Tatoeba result: lang: {} | acc: {:.4}".format("avg", sum(accs)/len(accs))
            print_rank_0(log_string)
            save_rank_0(args, log_string)
