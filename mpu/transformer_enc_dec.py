...
class Adapter(nn.Module):
    def __init__(
        ...
        # adpater               
        self.down_p = RowParallelLinear(
            config.d_model,
            config.d_model // 32,
            # bias=False,
            init_method=output_layer_init_method)
        self.up_p = RowParallelLinear(
            config.d_model // 32,
            config.d_model,
            # bias=False,
            init_method=output_layer_init_method)
    ...
    def forward(self, hidden_states):
        ...
        forwarded_states = self.up_p(gelu(self.down_p(forwarded_states)))
...
class ParallelTransformer(nn.Module):
    def __init__(self, config: EncDecConfig, word_embeds: VocabParallelEmbedding, prompt_config=None, is_decoder=False, checkpoint_activations=False, checkpoint_num_layers=1):
        ...
        self.prompt_embeds = nn.Embedding(1, config.d_model)
        if self.prompt_config is not None:
            self.prompt_len = prompt_config["prompt_len"]
            if self.prompt_len: self.prompt_embeds = nn.Embedding(self.prompt_len, config.d_model)
    ...
    def init_prompt_embeds(self):
        if self.prompt_config is not None:
            ...
            if self.prompt_len: self.prompt_embeds.weight.data = prompt_weights

    def get_input_embeds(self, input_ids):
        if self.prompt_config is None:
            return self.word_embeds(input_ids)
        input_embs = torch.empty(*input_ids.size(), self.d_model, dtype=torch.float16).to(input_ids.device)
        input_embs[input_ids>=0] = self.word_embeds(input_ids[input_ids>=0])
        if self.prompt_len: input_embs[input_ids<0] = self.prompt_embeds(-(input_ids[input_ids<0]+1))
        return input_embs # bs * seq_len * hidden
